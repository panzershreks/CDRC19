...
rf_predictrors <- clean_healthcare_no_res


# We impute the missing data using Random Forest
set.seed(100)
healthcare_rf_df <- rfImpute(x = as.data.frame(rf_predictrors), y = total_confirmed_deaths_due_to_covid_19_per_million_people)
any(is.na(healthcare_rf_df)) # check that no NAs
healthcare_rf_df <- subset(healthcare_rf_df, select = -1)

write.csv(healthcare_rf_df, file="GLM (Random Forest (2nd))/RF Split Into Categories Initial Work/RF Category Imputed Full CSV/RF_healthcare_full_imputed.csv", row.names=FALSE)
...
write.csv(sig_healthcare_imputed, file="GLM (Random Forest (2nd))/RF Split Into Categories Initial Work/RF Category Sig Imputed CSV/RF_sig_healthcare_imputed.csv", row.names=FALSE)
write.csv(sig_healthcare_missing, file="GLM (Random Forest (2nd))/RF Split Into Categories Initial Work/RF Category Sig Miss CSV/RF_sig_healthcare_missing.csv", row.names=FALSE)
