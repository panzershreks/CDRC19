...
# impute
demographic_rf <- missForest(data.frame(demographic_all_missing))
demographic_rf_df <- as.data.frame(demographic_rf$ximp)
any(is.na(demographic_rf_df)) # check that no NAs
write.csv(demographic_rf_df, file="GLM Take 2/Split Into Categories Initial Work/Category Imputed Full CSV/demographic_full_imputed.csv", row.names=FALSE)

# insert response column
full_imputed_demographic <- cbind(response_variable, demographic_rf_df)
...
write.csv(sig_demog_imputed, file="GLM Take 2/Split Into Categories Initial Work/Category Sig Imputed CSV/sig_demog_imputed.csv", row.names=FALSE)
write.csv(sig_demog_missing, file="GLM Take 2/Split Into Categories Initial Work/Category Sig Miss CSV/sig_demog_missing.csv", row.names=FALSE)
