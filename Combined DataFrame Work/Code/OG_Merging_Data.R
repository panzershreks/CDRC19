...
updated_Diabetes_Prevalence_NCD_RisC_2017_ <- Diabetes_Prevalence_NCD_RisC_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% summarise_all(funs( na.omit(unique(.)) )) %>% slice_max(Year)
d1 <- subset(updated_Diabetes_Prevalence_NCD_RisC_2017_, select=-c(Year))


updated_Diabetes_prevalence_relative_to_obesity_overweight_OWID_based_on_WHO_and_WDI <- Diabetes_prevalence_relative_to_obesity_overweight_OWID_based_on_WHO_and_WDI %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% summarise_all(funs( na.omit(unique(.)) ))%>% slice_max(Year)
d2 <- subset(updated_Diabetes_prevalence_relative_to_obesity_overweight_OWID_based_on_WHO_and_WDI, select=-c(Year))

updated_Diarrheal_disease_deaths_in_70_age_group_by_attributable_to_risk_factors_IHME_2018_ <- Diarrheal_disease_deaths_in_70_age_group_by_attributable_to_risk_factors_IHME_2018_ %>% group_by(Entity) %>%fill(everything(), .direction = "updown")%>% slice_max(Year)
d3 <- subset(updated_Diarrheal_disease_deaths_in_70_age_group_by_attributable_to_risk_factors_IHME_2018_, select=-c(Year))

updated_Male_lung_cancer_deaths_per_100_000_WHO_IARC_2016_ <- Male_lung_cancer_deaths_per_100_000_WHO_IARC_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
d4 <- subset(updated_Male_lung_cancer_deaths_per_100_000_WHO_IARC_2016_, select=-c(Year))

updated_Non_communicable_disease_age_standardized_death_rates_IHME_2017_ <- Non_communicable_disease_age_standardized_death_rates_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
d5 <- subset(updated_Non_communicable_disease_age_standardized_death_rates_IHME_2017_, select=-c(Year))

updated_Obesity_Prevalence_WHO_2019_ <- Obesity_Prevalence_WHO_2019_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
d1 <- subset(na.omit(Affordability_of_diets_SOFI_2021_), select=-c(Year))
d2 <- na.omit(Number_of_people_with_and_without_access_to_improved_sanitation_OWID_based_on_WDI) %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
ws2 <- na.omit(World_Happiness_Report_2021_) %>% group_by(Entity) %>% slice_max(Year)
world_stats <- subset(ws2, select=-c(Year))
miss_var_summary(world_stats)

# Covid Data


# Vaccine Eligibility taken out as lots of missing values for most things...
...
updated_COVID_testing_time_series_data <- COVID_testing_time_series_data %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_COVID_19_Variants <- COVID_19_Variants %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_COVID_19_Tests_per_million_people <- COVID_19_Tests_per_million_people %>% fill(everything(), .direction = "updown")%>% group_by(Entity) %>% slice_max(Year)
...
updated_COVID_19_Tests <- COVID_19_Tests %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
c5 <- subset(updated_COVID_19_Tests, select=-c(Year))


COVID_2019_ECDC_2020_ <- read_csv("Working_Data/covid/COVID-2019 - ECDC (2020).csv")
updated_COVID_2019_ECDC_2020_ <- COVID_2019_ECDC_2020_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_COVID_2019_Hospital_ICU <- COVID_2019_Hospital_ICU %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_State_of_Vaccine_Confidence_Larson_et_al_2016_ <- State_of_Vaccine_Confidence_Larson_et_al_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% summarise_all(funs( na.omit(unique(.)) )) %>% slice_max(Year)
...
updated_Child_mortality_estimates_Gapminder_2015_ <- Child_mortality_estimates_Gapminder_2015_  %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Child_mortality_1950_2017_IHME_2017_ <- Child_mortality_1950_2017_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Excess_Mortality_Data_OWID_2021_ <- Child_mortality_1950_2017_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Female_and_male_life_expectancy_at_birth_OWID_based_on_UN_Population_Division_2017_ <- Female_and_male_life_expectancy_at_birth_OWID_based_on_UN_Population_Division_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Inequality_in_human_development_indices_UNDP_2019_ <- Inequality_in_human_development_indices_UNDP_2019_ %>% 
  group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Infant_mortality_rate_IHME_2017_ <- Infant_mortality_rate_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_International_Historical_Statistics_Deaths_per_1_000_Brian_Mitchell_2013_ <- International_Historical_Statistics_Deaths_per_1_000_Brian_Mitchell_2013_ %>% group_by(Entity)%>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Life_expectancy_population_Gapminder_2019_UN_2019_and_Our_World_In_Data_2019_ <- Life_expectancy_population_Gapminder_2019_UN_2019_and_Our_World_In_Data_2019_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Life_Expectancy_1950_2015_UN_Population_Division_2015_ <- Life_Expectancy_1950_2015_UN_Population_Division_2015_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Male_and_female_life_expectancy_by_age_in_the_long_run_Human_Mortality_Database_2018_and_others_ <- Male_and_female_life_expectancy_by_age_in_the_long_run_Human_Mortality_Database_2018_and_others_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Mean_BMI_NCD_RisC_2017_ <- Mean_BMI_NCD_RisC_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Neonatal_Mortality_Rate_via_Childmortality_org_2015_ <- Neonatal_Mortality_Rate_via_Childmortality_org_2015_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Number_of_child_deaths_1950_2017_IHME_2017_ <- Number_of_child_deaths_1950_2017_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Number_of_infant_deaths_IHME_2017_ <- Number_of_infant_deaths_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Number_of_neonatal_deaths_IHME_2017_ <- Number_of_neonatal_deaths_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Population_Gapminder_HYDE_UN_ <- Population_Gapminder_HYDE_UN_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
miss_var_summary(demographic_df)
...
updated_Air_pollution_deaths_breakdown_by_age_IHME <- Air_pollution_deaths_breakdown_by_age_IHME %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Deaths_attributed_to_air_pollution_Lelieveld_et_al_2019_ <- Deaths_attributed_to_air_pollution_Lelieveld_et_al_2019_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Average_monthly_incomes_or_consumption_by_decile_and_quintile_PovcalNet_2019_ <- Average_monthly_incomes_or_consumption_by_decile_and_quintile_PovcalNet_2019_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Country_Income_Classification_World_Bank_2017_ <- Country_Income_Classification_World_Bank_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_economic_inequality_gini_index <- economic_inequality_gini_index %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_GDP_growth_from_previous_year_2020_Q2_Eurostat_OECD_National_sources_ <- GDP_growth_from_previous_year_2020_Q2_Eurostat_OECD_National_sources_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_GDP_per_capita_PPP_2011_WDI_2016_ <- GDP_per_capita_PPP_2011_WDI_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_GNP_by_country_2021 <- GNP_by_country_2021 %>% group_by(country) %>% fill(everything(), .direction = "updown")
...
updated_Income_Inequality_World_Bank_2016_ <- Income_Inequality_World_Bank_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Maddison_Project_Database_2020_Bolt_and_van_Zanden_2020_ <- Maddison_Project_Database_2020_Bolt_and_van_Zanden_2020_ %>% fill(everything(), .direction = "updown")%>% group_by(Entity) %>% slice_max(Year)
...
updated_Measures_and_indicators_for_Poverty_PovcalNet_World_Bank_2017_ <- Measures_and_indicators_for_Poverty_PovcalNet_World_Bank_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_National_Poverty_Lines_Jolliffe_and_Prydz_2016_ <- National_Poverty_Lines_Jolliffe_and_Prydz_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_OPHI_Multidimensional_Poverty_Index_Alkire_and_Robles_2016_ <- OPHI_Multidimensional_Poverty_Index_Alkire_and_Robles_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_PolcalNet_Global_Poverty_2017_ <- PolcalNet_Global_Poverty_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Poverty_rate_50_of_median_LIS_Key_Figures_2018_ <- Poverty_rate_50_of_median_LIS_Key_Figures_2018_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
economic_df <- subset(economic_df, select=-c(`Closing the poverty gap including/excluding China (PovcalNet (World Bank) (2017))`))

miss_var_summary(economic_df)
...
updated_Attitudes_to_Vaccines_Wellcome_Trust_2019_ <- Attitudes_to_Vaccines_Wellcome_Trust_2019_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Clark_Flèche_Senik_Happiness_Inequality <- Clark_Flèche_Senik_Happiness_Inequality %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Disability_Adjusted_Life_Years_WHO_2015_ <- Disability_Adjusted_Life_Years_WHO_2015_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Health_Coverage_ILO_2014_ <- Health_Coverage_ILO_2014_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Health_Expenditure_Tanzi_Schuknecht_2000_ <- Health_Expenditure_Tanzi_Schuknecht_2000_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Health_Expenditure_and_Financing_OECDstat_2017_ <- Health_Expenditure_and_Financing_OECDstat_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Health_expenditure_per_capita_World_Bank_WDI_2018_ <- Health_expenditure_per_capita_World_Bank_WDI_2018_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Health_expenditure_per_capita_PPP_World_Bank_2016_ <- Health_expenditure_per_capita_PPP_World_Bank_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Healthcare_Access_and_Quality_Index_IHME_2017_ <- Healthcare_Access_and_Quality_Index_IHME_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Healthcare_capacity_OECD_2020_ <- Healthcare_capacity_OECD_2020_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Healthy_Life_Expectancy_IHME <- Healthy_Life_Expectancy_IHME %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Long_run_series_of_health_expenditure_World_Bank_WDI_2017_ <- Long_run_series_of_health_expenditure_World_Bank_WDI_2017_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_OECD_Social_Spending_Health <- OECD_Social_Spending_Health %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_OECD_Social_Spending_Old_Age <- OECD_Social_Spending_Old_Age %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Out_of_pocket_expenditure_per_capita_on_healthcare_WHO_Global_Health_Expenditure <- Out_of_pocket_expenditure_per_capita_on_healthcare_WHO_Global_Health_Expenditure %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Percentage_of_persons_without_health_insurance_Council_of_Economic_Advisers_and_National_Center_fo <- Percentage_of_persons_without_health_insurance_Council_of_Economic_Advisers_and_National_Center_fo %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Perceptions_of_spending_on_health_expenditure_IPSOS_2016_ <- Perceptions_of_spending_on_health_expenditure_IPSOS_2016_ %>% group_by(Entity) %>% fill(everything(), .direction = "updown")%>% slice_max(Year)
...
updated_Public_expenditure_on_health_GDP_OWID_based_on_WHO_and_historical_estimates <- Public_expenditure_on_health_GDP_OWID_based_on_WHO_and_historical_estimates %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
updated_Total_gross_official_disbursements_for_medical_research_and_basic_heath_sectors_OECD <- Total_gross_official_disbursements_for_medical_research_and_basic_heath_sectors_OECD %>% group_by(Entity) %>% fill(everything(), .direction = "updown") %>% slice_max(Year)
...
miss_var_summary(healthcare_df)
